...
def _row_to_event(r: db.Row) -> Event:
    return Event(
...
def for_entity(conn: db.Connection, entity_type: str, entity_id: str) -> list[Event]:
...
def by_seq(conn: db.Connection, seqs: list[int]) -> list[Event]:
